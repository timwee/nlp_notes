...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    # one way is to dist(x, y) = sqrt(dot(x, x) - 2 * dot(x, y) + dot(y, y)) in sklearn
    #dists = cdist(X, self.X_train, metric='euclidean')

    dists = np.dot(X, self.X_train.T)
    dists *= -2
    XX = np.einsum('ij,ij->i', X, X)[:,np.newaxis]
    XtrainXtrain = np.einsum('ij,ij->i', self.X_train, self.X_train)[np.newaxis,:]
    dists += XX
    dists += XtrainXtrain
    ...
    #                         END OF YOUR CODE                              #
    #########################################################################
    return np.sqrt(dists, out=dists)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      candidate_labels = self.y_train[sorted_idx]
      #########################################################################
      # TODO:                                                                 #
      ...
      # label.                                                                #
      #########################################################################
      y_pred[i] = np.argmax(np.bincount(candidate_labels))
